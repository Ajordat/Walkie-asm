...
SIZEL EQU 0x01
WORD EQU 0x02	;VARIABLE PER A LLEGIR LA TRANSMISSIÃ“ REBUDA
TIMES EQU 0x03	;VARIABLE PER A CONTROLAR EL REBOT DELS POLSADORS
 
;VARIABLES LEDS
HORIZ EQU 0x04		;QUAN ESTÃ€ ACTIVA REALITZA LA RUTINA DE "ROTACIÃ“" DELS LEDS
LEDS_HZ EQU 0x05	;VARIABLE PER A CONTROLAR LA FREQÃœÃˆNCIA DELS LEDS
RIGHT_NOTLEFT EQU 0x06	;INDICA EL SENTIT DE LA ROTACIÃ“ DELS LEDS 
BLINK EQU 0x07		;VARIABLE PER A CONTROLAR LA FREQÃœÃˆNCIA DELS LEDS
RES_DIV EQU 0x08	;EMMAGATZEMEM EL RESULTAT DE LA DIVISIÃ“ PER 10
...
WAIT EQU 0x09	;FLAG PER A ESPERAR A LA INTERRUPCIÃ“ PER LA TRANSMISSIÃ“ RF

;VARIABLES BOTONS
F_REBOTS_LOAD EQU 0x0A	;FLAG PELS REBOTS DEL BOTÃ“ DE LOAD
F_REBOTS_SEND EQU 0x0B	;FLAG PELS REBOTS DEL BOTÃ“ DE SEND
LOADED EQU 0x0C	    ;FLAG PER SABER SI S'HA FET L'ACCIÃ“ DE CARREGAR 
SENT EQU 0x0D	    ;FLAG PER SABER SI S'HA FET L'ACCIÃ“ D'ENVIAR 
...
SEND_BYTE EQU 0xEE  ;BYTE QUE S'ENVIA DE LA PIC AL PC PER A DEMANAR UN BYTE MÃ‰S DE LA CADENA. EQUIVALENT A UN ACK.
END_BYTE EQU 0xFF   ;BYTE QUE S'ENVIA DEL PC A LA PIC PER A INDICAR QUE JA S'HA ENVIAT TOTA LA CADENA. EQUIVALENT A UN FIN.
SEG_10_H EQU 0x07   ;NOMBRE DE COPS QUE HA DE SALTAR LA INTERRUPCIÃ“ PER A QUE PASSIN 10 SEGONS (PART ALTA)
SEG_10_L EQU 0xD0   ;NOMBRE DE COPS QUE HA DE SALTAR LA INTERRUPCIÃ“ PER A QUE PASSIN 10 SEGONS (PART BAIXA)
 
;*********************************
; VECTORS DE RESET I INTERRUPCIÃ“ *
;*********************************
    ORG 0x000000
RESET_VECTOR
    goto MAIN
...
;***********************************
;* RUTINES DE SERVEI D'INTERRUPCIÃ“ *
;***********************************

    ;* 
    ; *   A LA INTERRUPCIÃ“ TORNEM A INICIAR EL TIMER PER A QUE TORNI A SALTAR AQUESTA, FIQUEM A 0 EL FLAG WAIT I 
    ; *   INCREMENTEM ELS COMPTADORS QUE S'UTILITZEN A LES SEGÃœENTS FUNCIONALITATS: CONTROLAR ELS REBOTS, CONTROLAR LA
    ; *   FREQÃœÃˆNCIA DELS LEDS I ESPERAR 10 SEGONS
    ; */
HIGH_INT
    call INIT_TMR
    incf TIMES, 1, 0
...
INIT_INTS
    bcf RCON, IPEN, 0	;ACTIVEM NOMÃ‰S LA INTERRUPCIÃ“ DEL TIMER0
    movlw 0xA0
    movwf INTCON, 0
    movlw 0x88
    movwf T0CON, 0
...
CHECK_LOAD_BTN
    ;COMPROVEM SI EL POLSADOR DE LOAD ESTÃ€ ACTIU
    btfsc PORTC, 1, 0
    goto LOAD_BTN
    ;SI NO ESTÃ€ ACTIU, BAIXEM EL FLAG QUE INDICA QUE JA HEM FET LA TASCA DE CARREGAR
    clrf LOADED, 0
    ;goto CHECK_SEND_BTN ?
...
CHECK_LOAD_15MS
    ...
    cpfsgt TIMES, 0
    goto CHECK_SEND_BTN
    ;SI JA HA PASSAT EL TEMPS TORNEM A MIRAR SI DESPRÃ‰S DELS REBOTS EL POLSADOR SEGUEIX ACTIU O NO
    ;SI NO HO ESTÃ€, ES TRACTAVA D'UN REBOT, ALTRAMENT ENS HAVIEN PITJAT
    clrf F_REBOTS_LOAD, 0
    btfss PORTC, 1, 0
    goto CHECK_SEND_BTN
    ;COM QUE ES TRACTA D'UNA INSTRUCCIÃ“ DE CARREGAR:
    ;ENVIEM UNA PETICIÃ“ DE CADENA A L'ORDINADOR
    movlw SEND_BYTE
    movwf TXREG, 0
...
CHECK_SEND_BTN
    ;COMPROVEM SI EL POLSADOR DE SEND ESTÃ€ ACTIU
    btfsc PORTC, 0, 0
    goto SEND_BTN
    ;SI NO ESTÃ€ ACTIU, BAIXEM EL FLAG QUE INDICA QUE JA HEM FET LA TASCA D'ENVIAR
    clrf SENT, 0
    ;goto BUCLE ?
...
SEND_BTN
    ...
    ;SITUACIÃ“ EN LA QUE HEM REBUT UN BYTE PER LA SIO
RX_PC
    ...
    movlw INIT_FSR0H
    movwf FSR0H, 0
    ;LLEGIM EL BYTE REBUT I MIREM SI ES TRACTA DEL CAS DE CARREGAR UNA CADENA A LA RAM O BÃ‰
    ...
    ;COM QUE UTILITZEM UN PROTOCOL ORIENTAT A CONNEXIÃ“, S'ESTABLEIX UN DIALEG, AIXÃ QUE PER ASSEGURAR QUE NO 
    ;ES PERDRÃ€ CAP BYTE L'ORDINADOR S'HA D'ESPERAR A QUE LA PIC LI CONTESTI
DEMANA_CHAR
    ;TRANSMETEM A L'ORDINADOR EL BYTE SEND_BYTE QUE VOL DIR QUE LI DEMANEM UN ALTRE CARÃ€CTER DE LA CADENA
    movlw SEND_BYTE
    movwf TXREG, 0
    ...
    movlw END_BYTE
    cpfseq WORD, 0
    ;SI ENCARA NO S'HA ACABAT LA CADENA, GUARDEM EL CARÃ€CTER REBUT A LA RAM
    goto GUARDA_CHAR
    ;SI S'HA ACABAT LA CÃ€RREGA FEM PARPALLEJAR ELS LEDS A 5HZ
    movlw F_5HZ
    movwf BLINK, 0
    clrf LATD, 0
    clrf LATB, 0
...
GUARDA_CHAR
    ...
    ;SITUACIÃ“ EN LA QUE HEM DE TRANSMETRE EL CONTINGUT DE LA RAM PER RF
SEND_MESSAGE
    ;SI HEM REBUT L'INSTRUCCIÃ“ DE TRANSMISSIÃ“ PER LA SIO LI HEM DE CONTESTAR
    movlw SEND_BYTE
    movwf TXREG, 0
...
SEND_RAM
    ...
    call RF_1
    ;ROTEM LA PARAULA A ENVIAR CAP A L'ESQUERRA (TRANSMETEM DE MÃ‰S A MENYS PES)
    rlcf WORD, 1, 0
    btfsc STATUS, C, 0
    ;SI EL PRIMER BIT Ã‰S UN 1 JA PODEM TRANSMETRE LA RESTA DE PARAULES
    goto ENVIA_TRAMA
    ;SI EL PRIMER BIT Ã‰S UN 0 ENVIEM UN 0 ABANS DE TRANSMETRE LA RESTA DE PARAULES
    call RF_0
    
ENVIA_TRAMA
    ;INICIALITZEM L'ÃNDEX DELS BITS DE LA PARAULA A 1 PERQUÃˆ JA HEM ENVIAT EL PRIMER
    movlw 0x01
    movwf INDEX, 0
    ;INICIALITZEM L'ÃNDEX DE LES PARAULES A TRANSMETRE A 0. LA TRANSMISSIÃ“ ACABARÃ€ QUAN AQUEST COMPTADOR VALGUI
    ;EL MATEIX QUE LA MIDA DE LA CADENA
    clrf N_SIZEL, 0
    clrf N_SIZEH, 0
    
CHECK_SIZE
    ;COMPAREM SI JA HEM TRANSMÃˆS TOTES LES LLETRES
    movf SIZEH, 0, 0
    cpfseq N_SIZEH, 0
    goto BUCLE_WORD
    movf SIZEL, 0, 0
    cpfslt N_SIZEL, 0
    ;SI L'ÃNDEX HA SOBREPASSAT LA MIDA DEL MISSATGE VOL DIR QUE JA HEM ACABAT.
    goto END_ENVIA_TRAMA
    
BUCLE_WORD
    ;SI NO HEM ACABAT DE TRANSMETRE UNA PARAULA ENVIEM EL SEGÃœENT BIT
    rlcf WORD, 1, 0
    btfss STATUS, C, 0
    ;SI ES TRACTA D'UN 0, PRIMER ENVIEM UN NIVELL A 5V I DESPRÃ‰S UN A 0V
    goto SEND_0
    
    ;SI ES TRACTA D'UN 1, PRIMER ENVIEM UN NIVELL A 0V I DESPRÃ‰S UN A 5V
SEND_1
    call RF_0
    call RF_1
    goto CHECK_WORD
...
CHECK_WORD
    ...
    movlw 0x08
    cpfseq INDEX, 0
    ;SI NO HEM ACABAT AMB LA PARAULA ANEM A TRANSMETRE EL SEGÃœENT BIT
    goto BUCLE_WORD
    ;SI HEM ACABAT RESETEJEM L'ÃNDEX DELS BITS D'UN BYTE I AUGMENTEM L'ÃNDEX DE LLETRES TRANSMESES
    clrf INDEX, 0
    incf N_SIZEL, 1, 0
    btfsc STATUS, C, 0
    incf N_SIZEH, 1, 0
    movff POSTINC0, WORD
    ;LEDS
    ;AUGMENTEM TAMBÃ‰ L'ÃNDEX DE LA PROPORCIÃ“ DE LLETRES TRANSMESES.
    ;QUAN AQUEST ÃNDEX ARRIBA AL RESULTAT DE LA DIVISIÃ“ VOL DIR QUE HEM D'ENCENDRE UN LED MÃ‰S
    incf N_LEDS, 1, 0
    movf N_LEDS, 0, 0
    cpfseq RES_DIV, 0
    ;SI NO HEM D'ENCENDRE UN ALTRE LED TORNEM A AGAFAR UN NOU BYTE DE LA RAM
    goto CHECK_SIZE    
    ;SI EL BIT DE MÃ‰S PES DEL PORTD ESTÃ€ ACTIU VOL DIR QUE JA HEM OMPLERT UN REGISTRE, AIXÃ QUE HEM DE MOURE 
...
NEXT_LED
    ;PER A ENCENDRE PROGRESSIVAMENT ELS LEDS ACTIVEM EL BIT DE MÃ‰S PES I ROTEM SENSE CARRY CAP A L'ESQUERRA
    bsf LATD, 7, 0
    rlncf LATD, 1, 0
    clrf N_LEDS, 0
    goto CHECK_SIZE
...
END_ENVIA_TRAMA
    ;SI JA HEM ACABAT DE TRANSMETRE L'ÃšLTIM BIT DE L'ÃšLTIM BYTE ENS ESPEREM A QUE AQUEST ESTIGUI EL SEU TEMPS EN
    ;L'ESTAT PERTINENT I DESPRÃ‰S BAIXEM EL SENYAL DE TRANSMISSIÃ“
    call RF_0
